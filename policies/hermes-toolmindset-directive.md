...
4. Use Hermes native `read_file` / `search_files` only when the wrapper is missing, broken, or the requested operation is genuinely simpler and lower risk.
5. Do not use native `mcp__*` tools. Prefer `cli-anything-*`; fallback to `mcp2cli` only if wrapper missing/broken.

This is routing policy for Workspace/API sessions. Report fallback honestly if a wrapper is missing or fails.
